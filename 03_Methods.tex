...
\begin{figure}
    \centering
    \includegraphics[width=.8\textwidth]{figures_method/DSwithBoundingBoxes.pdf}
    \caption{Caption}
...
\begin{figure}
    \centering
    \includegraphics[width=.8\textwidth]{figures_method/LocalLinearCorrelationsWithCorrs.pdf}
    \caption{Caption}
    \label{fig:my_label}
\end{figure}
...
To combine the found localized correlations to a global correlation clustering, we evaluate all the localized correlations without its bounding boxes. Since we can represent the localized correlations as hyperplanes with a unit normal vector $\vec{n}$ and a shortest distance to origin $\delta$ (c.f. \autoref{eq:highhnf}), we can easily compare different hyperplanes by variation in rotation(angles), using the cosine-similarity between the normal vectors, and translation(absolute positioning), using the euclidean-distance between the shortest distances. For adjustability we introduce two parameters $thres_{\Delta \text{cos-dist}}, thres_{\Delta \text{cos-dist}} \in [0,1]$ and $thres_{\Delta \text{eucl-dist}}, thres_{\Delta \text{eucl-dist}} \in \N$, which serves as thresholds for maximal allowed divergence of both measures. To formalize the requirements for a successful combination (\textit{stitching}) of local correlations we introduce following relations:\todor{definitionen einfuehren oder textuell einfach beschreiben?}

\subsubsection*{Orientation-Similarity}
Given a set of unique linear correlations $C$ in \gls{hnf}. Then two correlations $c_{a,\delta_a} \in C$ and $c_{b,\delta_b} \in C$ are \textit{orientation-similar}, if the absolute cosine-similarity between the correlations normal vectors $a$ and $b$ is bigger than $1 - thres_{\Delta \text{cos-dist}}$.
\begin{align}\label{eq:cosdist}
    |\text{cos-sim}(a,b)| = \left|\frac{\sum_{i=1}^{n} a_{i} b_{i}}{\sqrt{\sum_{i=1}^{n} a_{i}^{2}} \sqrt{\sum_{i=1}^{n} b_{i}^{2}}}\right| > 1 - thres_{\Delta \text{cos-dist}}
\end{align}

\subsubsection*{Translation-Similarity}
Given a set of unique linear correlations $C$ in \gls{hnf}. Then two correlations $c_{a,\delta_a} \in C$ and $c_{b,\delta_b} \in C$ are \textit{translation-similar}, the absolute euclidean-distance between the correlations distances $\delta_a$ and $\delta_b$ is smaller than $thres_{\Delta \text{eucl-dist}}$.
%  if orientation-similarity (c.f. \autoref{eq:cosdist}) holds and
\begin{align}\label{eq:eucldist}
    |\text{eucl-dist}(\delta_a,\delta_b)| < thres_{\Delta \text{eucl-dist}}
\end{align}
...
\begin{align}
    R^{o}=\{\{x,y\} \in C \times C\: \big|\: |\text{cos-sim}(x,y)| > 1-thres_{\Delta \text{cos-dist}}\}\\
    R^{t}=\{\{x,y\} \in C \times C\: \big|\: |\text{eucl-dist}(x,y)| < thres_{\Delta \text{eucl-dist}}\}
\end{align}
...
\begin{figure}
    \centering
    \includegraphics[width=.8\textwidth]{figures_method/StitchedLinearCorrelationsWithCorrs.pdf}
    \caption{Caption}
    \label{fig:my_label}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=.8\textwidth]{figures_method/RelabeledCorrelationsWithCorrsL.pdf}
    \caption{Caption}
    \label{fig:my_label}
\end{figure}
...
\begin{itemize}[label={\tiny\raisebox{1ex}{\textbullet}},topsep=6pt,itemsep=-1ex,partopsep=2ex,parsep=2ex]
    \item \text{Partitioning:} $MinPts$ and $\epsilon$-distance for \gls{optics} ordering and a $reachability-thres$ to split the ordering after significant jumps
    \item \text{Local Correlation Clustering:} $s$ splits, $m$ minimum points and $j$ jitter for \gls{cash}
    \item \text{Stitching:} $thres_{\Delta cos-dist}$ and $thres_{\Delta eucl-dist}$ for similarity constraints
    \item \text{Relabeling:} $j$ jitter of local Correlation Clustering aswell
\end{itemize}
...
Partitioning                 & \begin{tabular}[c]{@{}l@{}}$\epsilon$ : Epsilon-distance\\ $MinPts$: min \# of pts in dense neighborhood\end{tabular} & $\mathcal{O}(|DS|^2)$          \\ \midrule
...
Stitching                    & \begin{tabular}[c]{@{}l@{}}$thres_{\Delta cos-dist}$: orientation threshold\\ $thres_{\Delta eucl-dist}$: distance threshold\end{tabular}  & $\mathcal{O}(c^2)$          \\ \midrule
Relabeling                   & $j$: jitter                                                  & $\mathcal{O}(|DS| \cdot c)$          \\ \bottomrule
\end{tabular}%
}
