...
\begin{itemize}
    \item \textbf{Partitioning Clustering:} has the objective of partitioning the whole data space into a predefined number of disjunct partitions optimizing a certain criterion function. It is one of the simplest and most fundamental version of cluster analysis \cite[Chapter 10.2]{han2011data}.\todor{citation mit gemischtem style? ist das erste buch, ohne seitennummer findet man das ja nie}
    ...
    Subspace Clustering algorithms can be divided into two categories: Finding \textit{axis-parallel} subspaces (\textit{Projected Clustering}) and finding \textit{arbitrarily oriented} subspaces (\textit{Correlation Clustering})\cite{zimek2009correlation}.\todor{L: Wie zitier ich das, wenn ich nur zu 66\% zustimme? :D Ich stimme einfach nicht mit der terminology zu. MMn sollte es nur axis-parallel(projected) und arbitrary oriented(correlation) geben}
...
\begin{tabular}{@{}|l|l|@{}}
\toprule
Method                   & General Characteristics                                                                                                                                                                                                                                                                                    \\ \midrule
...
Subspace Clustering      & \begin{tabular}[c]{@{}l@{}}\tabitem Find relevant subspaces and correlations\\ \tabitem Find clusters in relevant subspaces\end{tabular}                                                                                                                                                                                 \\ \bottomrule
\end{tabular}%
}
\caption{Overview of clustering methods based on objective. Note that some algorithms may
combine various methods. Inspired by \cite[Chapter 10.1.3]{han2011data}.}
...
\begin{tabular}{@{}|l|l|l|@{}}
\toprule
Method                   & General Characteristics                                                                                                                                                                                                                                                                                    & Implementations                                                                                                                                       \\ \midrule
...
Density-based Clustering & \begin{tabular}[c]{@{}l@{}}* Can find arbitrarily shaped clusters\\ * Clusters are dense regions of objects in space that are\\ separated by low-density regions\\ * Cluster density: Each point must have a minimum number of\\ points within its “neighborhood”\\ * May filter out outliers\end{tabular} & \begin{tabular}[c]{@{}l@{}}DBSCAN\cite{DBSCANEKSX96}\\ LSDBC\cite{biccici2007locally}\\ GriDBSCAN\cite{uncu2006gridbscan}\\ OPTICS\cite{opticsankerst1999optics}\end{tabular}                                                                           \\ \midrule
Hierarchical Clustering  & \begin{tabular}[c]{@{}l@{}}* Clustering is a hierarchical decomposition (i.e., multiple levels)\\ * Cannot correct erroneous merges or splits\\ * May incorporate other techniques like microclustering or\\ consider object “linkages”\end{tabular}                                                       & \begin{tabular}[c]{@{}l@{}}AGNES\cite[Chapter 5]{kaufman2009finding}\\ DIANA\cite[Chapter 6]{kaufman2009finding}\\ SLINK\cite{sibson1973slink}\\ OPTICS\cite{opticsankerst1999optics}\end{tabular}                                                                                \\ \midrule
Subspace Clustering      & \begin{tabular}[c]{@{}l@{}}* Find relevant subspaces and correlations\\ * Find clusters in relevant subspaces\end{tabular}                                                                                                                                                                                 & \begin{tabular}[c]{@{}l@{}}Axis-parallel:\\ \quad CLIQUE\cite{cliqueagrawal1998automatic}\\ \quad SUBCLU\cite{sublcukailing2004density}\\ \quad PROCLUS\cite{proclusaggarwal1999fast}\\ Arbitrarily oriented:\\ \quad 4C\cite{4cbohm2004computing}\\ \quad ORCLUS\cite{orclusaggarwal2000finding}\\ \quad COPAC\cite{copacachtert2007robust}\\ \quad ERiC\cite{ericachtert2007exploring}\\ \quad HiCO\cite{hicoachtert2006mining}\\ \quad CASH\cite{CASHachtert2008global}\end{tabular} \\ \bottomrule
\end{tabular}%
}
\caption{Overview of clustering methods discussed in this chapter. Note that some algorithms may
combine various methods.\todor{lieber ohne refs?}}
\label{tab:my-table}
\end{table}
